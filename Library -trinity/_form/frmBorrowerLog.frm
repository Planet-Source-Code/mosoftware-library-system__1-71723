...
Begin VB.Form frmBorrowerLog 
   ...
   Begin VB.Frame fraList 
      ...
      Begin VB.Label Label2 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "You can view Borrower's Log Time and Date what time he enter into Library."
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
      ...
      Begin VB.Image Image3 
         ...
         Index           =   1
         Left            =   120
         Picture         =   "frmBorrowerLog.frx":0000
         Stretch         =   -1  'True
         Top             =   120
         Width           =   600
      End
   ...
   End
End
Attribute VB_Name = "frmBorrowerLog"
...
Private Sub btnAll_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_borrower_log.log_id, tbl_borrowers.B_id, tbl_borrowers.fn, tbl_borrowers.mn, tbl_borrowers.ln, tbl_borrower_type.b_type, tbl_borrower_log.logtime, tbl_borrower_log.logdate " & _
                "FROM tbl_borrower_type INNER JOIN (tbl_borrowers INNER JOIN tbl_borrower_log ON tbl_borrowers.B_id = tbl_borrower_log.B_id) ON tbl_borrower_type.bt_id = tbl_borrowers.bt_id;"
        iStartPage(Index) = 1
...
Public Function PRINT_RECORD(Index As Integer)
    ...
    If Index = 0 Then
        ...
        Set dtrBLog.DataSource = rsView1
        dtrBLog.Show 1
    End If
End Function
...
Private Sub Form_Load()
    ...
    sSQL(0) = "SELECT tbl_borrower_log.log_id, tbl_borrowers.B_id, tbl_borrowers.fn, tbl_borrowers.mn, tbl_borrowers.ln, tbl_borrower_type.b_type, tbl_borrower_log.logtime, tbl_borrower_log.logdate " & _
        "FROM tbl_borrower_type INNER JOIN (tbl_borrowers INNER JOIN tbl_borrower_log ON tbl_borrowers.B_id = tbl_borrower_log.B_id) ON tbl_borrower_type.bt_id = tbl_borrowers.bt_id;"
    Lv_MainInfo
    lvList(0).Refresh
    frmMain.TabMainIni 2, "Borrowers Log", 16
    'ListOfDisabledBtn
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    frmMain.TabMainIni 1, "Borrowers Log", 16
End Sub
...
Public Sub Lv_MainInfo()
    sColWidth(0) = "1200,1800,1500,1500,1500,1500,1500,1500"
    sColumns(0) = "Log ID,Borrower ID,First Name,Middle Name,Last Name,B. Type,Log Time,Log Date"
    iIcon(0) = 16
    iLoop(0) = CountSplitItem(sColumns(0), ",")
    sFields(0) = "log_id,B_id,fn,mn,ln,b_type,logtime,logdate"
    sNoRec(0) = "No Current Borrowers Logs Records."
    iStartPage(0) = 1
    iNoPage(0) = 75
...
Public Function Execute_SearchItem(sFilter As String)
    ...
    sSQL(0) = "SELECT tbl_borrower_log.log_id, tbl_borrowers.B_id, tbl_borrowers.fn, tbl_borrowers.mn, tbl_borrowers.ln, tbl_borrower_type.b_type, tbl_borrower_log.logtime, tbl_borrower_log.logdate " & _
            "FROM (tbl_borrower_type INNER JOIN tbl_borrowers ON tbl_borrower_type.bt_id = tbl_borrowers.bt_id) INNER JOIN tbl_borrower_log ON tbl_borrowers.B_id = tbl_borrower_log.B_id " & _
            "WHERE " & sFilter & " " & _
            "GROUP BY tbl_borrower_log.log_id, tbl_borrowers.B_id, tbl_borrowers.fn, tbl_borrowers.mn, tbl_borrowers.ln, tbl_borrower_type.b_type, tbl_borrower_log.logtime, tbl_borrower_log.logdate;"
    LvRefresh 0
End Function
