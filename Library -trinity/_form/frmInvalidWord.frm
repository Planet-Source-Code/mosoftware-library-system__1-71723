...
Begin VB.Form frmInvalidWord 
   ...
   Begin VB.Frame fraList 
      ...
      Begin VB.Image Image3 
         ...
         Index           =   0
         Left            =   240
         Picture         =   "frmInvalidWord.frx":0000
         Top             =   120
         Width           =   720
      End
      ...
      Begin VB.Label Label2 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "You canv iew List of Invalid Words use for Searching of books."
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmInvalidWord"
...
Private Sub btnAll_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_invalid_words.InvalidWord " & _
            "FROM tbl_invalid_words;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, Index, sSQL(Index), iStartPage(Index), iNoPage(Index), iIcon(Index), sColumns(Index), iLoop(Index), sColWidth(Index), sFields(Index), "No Current Invalid Word(s) Records.")
    End If
End Sub
...
Private Sub btnEdited_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_invalid_words.InvalidWord " & _
                    ...
                    "WHERE (((tbl_invalid_words.DateModified) Between #" & Date & "# And #" & Date & "#)) " & _
                    "GROUP BY tbl_invalid_words.InvalidWord;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, Index, sSQL(Index), iStartPage(Index), iNoPage(Index), iIcon(Index), sColumns(Index), iLoop(Index), sColWidth(Index), sFields(Index), "No Current Updated Invalid Word(s) Records.")
    End If
End Sub
...
Public Function PRINT_RECORD(Index As Integer)
    ...
    If Index = 0 Then
        ...
        Set dtrInvalidWrd.DataSource = rsView1
        dtrInvalidWrd.Show 1
    End If
End Function
...
Private Sub Form_Load()
    ...
    sSQL(0) = "SELECT tbl_invalid_words.InvalidWord " & _
            "FROM tbl_invalid_words;"
    Lv_MainInfo
    lvList(0).Refresh
    frmMain.TabMainIni 2, "Invalid Words", 10
    'ListOfDisabledBtn
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    frmMain.TabMainIni 1, "Invalid Words", 10
End Sub
...
Private Sub lvList_DblClick(Index As Integer)
    If Index = 0 Then
        If iRec(Index) > 0 Then
            frmInvalidWord_AE.bStat = False
            Set frmInvalidWord_AE.fCur = Me
            frmInvalidWord_AE.Show 1
        End If
    End If
End Sub
...
Public Sub Lv_MainInfo()
    sColWidth(0) = "2000"
    sColumns(0) = "Invalid Words"
    iIcon(0) = 14
    iLoop(0) = CountSplitItem(sColumns(0), ",")
    sFields(0) = "InvalidWord"
    sNoRec(0) = "No Current Invalid Word(s) Records."
    iStartPage(0) = 1
    iNoPage(0) = 75
...
Public Function btnNew_Load(Index As Integer)
     If Index = 0 Then
        sSQL(Index) = "SELECT tbl_invalid_words.InvalidWord " & _
                    "From tbl_invalid_words " & _
                    "WHERE (((tbl_invalid_words.DateAdded) Between #" & Date & "# And #" & Date & "#)) " & _
                    "GROUP BY tbl_invalid_words.InvalidWord;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, 0, sSQL(0), iStartPage(0), iNoPage(0), iIcon(0), sColumns(0), iLoop(0), sColWidth(0), sFields(0), "No Current New Invalid Word(s) Added Records.")
    End If
End Function
    ...
    If iRec(Index) > 0 Then
        If Index = 0 Then
            ...
            If MsgBox(sMsgDel, vbQuestion + vbYesNo, "Delete") = vbYes Then
                DELETE_DATA "tbl_invalid_words", "InvalidWord", lvList(Index).SelectedItem.SubItems(1)
