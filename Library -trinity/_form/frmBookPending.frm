...
Begin VB.Form frmBookPending 
   ...
   Begin VB.Frame fraList 
      ...
      Begin VB.Label Label2 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "You can view list of Books Been Borrowed"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
      ...
      Begin VB.Image Image3 
         ...
         Index           =   1
         Left            =   120
         Picture         =   "frmBookPending.frx":0000
         Top             =   120
         Width           =   720
      End
   ...
   End
End
Attribute VB_Name = "frmBookPending"
...
Private Sub btnAll_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_reg_books.rb_id, tbl_reg_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks " & _
            "FROM tbl_books INNER JOIN tbl_reg_books ON tbl_books.isbn = tbl_reg_books.isbn " & _
            "WHERE (((tbl_reg_books.pending) Like '1')) " & _
            "GROUP BY tbl_reg_books.rb_id, tbl_reg_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks;"
        iStartPage(Index) = 1
...
Public Function LvNew(Index As Integer)
    If Index = 0 Then
        frmBookPending_AE.bStat = True
        Set frmBookPending_AE.fCur = Me
        frmBookPending_AE.Show 1
    End If
End Function
    ...
    If Index = 0 Then
        ...
        If isBarcodeBookExist(sBarcode, False) = True And Len(sBarcode) > 0 Then
            If isPending(sBarcode) = True Then
                UPDATE_DATA "tbl_reg_books", "pending", "0", sWhere, ",", True
                frmBookPending.Lv_MainInfo
            Else
                MsgBox "You cannot Activate this Book in Pending. Book is not in Pending.", vbExclamation, "ErrPending"
            End If
        Else
...
Public Function PRINT_RECORD(Index As Integer)
    ...
    If Index = 0 Then
        ...
        Set dtrPending.DataSource = rsView1
        dtrPending.Show 1
    End If
End Function
...
Private Sub Form_Load()
    ...
    sSQL(0) = "SELECT tbl_reg_books.rb_id, tbl_reg_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks " & _
            "FROM tbl_books INNER JOIN tbl_reg_books ON tbl_books.isbn = tbl_reg_books.isbn " & _
            "WHERE (((tbl_reg_books.pending) Like '1')) " & _
            "GROUP BY tbl_reg_books.rb_id, tbl_reg_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks;"
    Lv_MainInfo
    lvList(0).Refresh
    frmMain.TabMainIni 2, "Pending Books", 18
    'ListOfDisabledBtn
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    frmMain.TabMainIni 1, "Pending Books", 18
End Sub
...
Public Sub Lv_MainInfo()
    sColWidth(0) = "1700,1500,3500,1200,1200,1200,2200"
    sColumns(0) = "Reg. Book ID,ISBN,Title,Barcode,Borrow,Pending,Remarks"
    iIcon(0) = 18
    iLoop(0) = CountSplitItem(sColumns(0), ",")
    sFields(0) = "rb_id,isbn,title,barcode,borrow,pending,remarks,pending"
    sNoRec(0) = "No Current Books Been Pending Records."
    iStartPage(0) = 1
    iNoPage(0) = 75
...
Public Function Execute_SearchItem(sFilter As String)
    ...
    sSQL(0) = "SELECT tbl_reg_books.rb_id, tbl_reg_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks " & _
            "FROM tbl_books INNER JOIN tbl_reg_books ON tbl_books.isbn = tbl_reg_books.isbn " & _
            "WHERE (tbl_reg_books.pending) Like '1' AND (" & sFilter & ") " & _
            "GROUP BY tbl_reg_books.rb_id, tbl_reg_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks;"
    LvRefresh 0
End Function
