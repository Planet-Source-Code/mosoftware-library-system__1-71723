...
Begin VB.Form frmUser 
   ...
   Begin VB.Frame fraList 
      ...
      Begin VB.Label Label2 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "You can view list of User Information and their respective handle posistion."
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
Private Sub btnAll_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_users.uid, tbl_users.usrnme, tbl_users.completename, tbl_users.admn, tbl_users.cashr, tbl_users.transction, tbl_users.opac, tbl_users.stat " & _
                    "FROM tbl_users;"
        iStartPage(Index) = 1
...
Private Sub Form_Load()
    ...
    sSQL(0) = "SELECT tbl_users.uid, tbl_users.usrnme, tbl_users.completename, tbl_users.admn, tbl_users.cashr, tbl_users.transction, tbl_users.opac, tbl_users.stat " & _
            "FROM tbl_users;"
    Lv_MainInfo
    lvList(0).Refresh
    frmMain.TabMainIni 2, "Users", 10
    'ListOfDisabledBtn
End Sub
...
Private Sub lvList_DblClick(Index As Integer)
    If Index = 0 Then
        If iRec(Index) > 0 Then
            frmUser_AE.bStat = False
            Set frmUser_AE.fCur = Me
            frmUser_AE.Show 1
        End If
    End If
End Sub
...
Public Sub Lv_MainInfo()
    sColWidth(0) = "1800,2000,2500,1000,1000,1500,1000,1000"
    sColumns(0) = "User ID,Username,Complete Name,Admin,Cashier,Transaction,Opac,Status"
    iIcon(0) = 10
    iLoop(0) = CountSplitItem(sColumns(0), ",")
    sFields(0) = "uid,usrnme,completename,admn,cashr,transction,opac,stat"
    sNoRec(0) = "No Current Uesrs Records."
    iStartPage(0) = 1
    iNoPage(0) = 75
...
Private Sub btnEdited_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_users.uid, tbl_users.usrnme, tbl_users.completename, tbl_users.admn, tbl_users.cashr, tbl_users.transction, tbl_users.opac, tbl_users.stat " & _
                    "From tbl_users " & _
                    "WHERE (((tbl_users.DateModified) Between #" & Date & "# And #" & Date & "#)) " & _
                    "GROUP BY tbl_users.uid, tbl_users.usrnme, tbl_users.completename, tbl_users.admn, tbl_users.cashr, tbl_users.transction, tbl_users.opac, tbl_users.stat;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, Index, sSQL(Index), iStartPage(Index), iNoPage(Index), iIcon(Index), sColumns(Index), iLoop(Index), sColWidth(Index), sFields(Index), "No Current Users Updated Info Records.")
    End If
End Sub
...
Public Function btnNew_Load(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_users.uid, tbl_users.usrnme, tbl_users.completename, tbl_users.admn, tbl_users.cashr, tbl_users.transction, tbl_users.opac, tbl_users.stat " & _
                    "From tbl_users " & _
                    "WHERE (((tbl_users.DateAdded) Between #" & Date & "# And #" & Date & "#)) " & _
                    "GROUP BY tbl_users.uid, tbl_users.usrnme, tbl_users.completename, tbl_users.admn, tbl_users.cashr, tbl_users.transction, tbl_users.opac, tbl_users.stat;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, 0, sSQL(0), iStartPage(0), iNoPage(0), iIcon(0), sColumns(0), iLoop(0), sColWidth(0), sFields(0), "No Current New Users Added Records.")
    End If
End Function
    ...
    If iRec(Index) > 0 Then
        If Index = 0 Then
            ...
            If MsgBox(sMsgDel, vbQuestion + vbYesNo, "Delete") = vbYes Then
                DELETE_DATA "tbl_users", "uid", lvList(Index).SelectedItem.SubItems(1)
