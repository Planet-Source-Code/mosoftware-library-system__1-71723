...
Begin VB.Form frmBookReg_AE 
   ...
   Begin VB.Frame fraList 
      ...
      Begin VB.Image Image1 
         ...
         Index           =   0
         Left            =   480
         Picture         =   "frmBookReg_AE.frx":0000
         Top             =   240
         Width           =   720
      End
   ...
   End
End
Attribute VB_Name = "frmBookReg_AE"
...
Public Function InsertData()
    ...
    If isFill(1) = True Then
            ...
            If isRecordExist(sChkSql) = False Then
                ...
                frmBook.btnNew_Load 1
                LvSearchItem frmBook.lvList(1), txtInput(1).Text
                Unload Me
            Else
                imgChk(0).Picture = frmMain.i16x16.ListImages(7).ExtractIcon
...
Public Function ModifyData()
    ...
    sChkSql = "SELECT tbl_reg_books.rb_id " & _
            ...
            "GROUP BY tbl_reg_books.rb_id;"
    If isFill(1) = True Then
        sWhere = "rb_id like '" & frmBook.lvList(1).SelectedItem.SubItems(1) & "'"
        sValues = txtInput(2).Text & "," & txtInput(3).Text _
                    & "," & Date & "," & sUserId
            If isRecordExist(sChkSql) = False Or txtInput(1).Text = frmBook.lvList(1).SelectedItem.SubItems(1) Then
                imgChk(0).Picture = frmMain.i16x16.ListImages(12).ExtractIcon
                UPDATE_DATA "tbl_reg_books", "barcode,remarks,DateModified,LastUserFK", sValues, sWhere, ",", True
                With frmBook.lvList(1)
                    .SelectedItem.SubItems(1) = txtInput(1).Text
                    .SelectedItem.SubItems(2) = txtInput(2).Text
                    .SelectedItem.SubItems(5) = txtInput(3).Text
                End With
                Unload Me
            Else
                imgChk(0).Picture = frmMain.i16x16.ListImages(7).ExtractIcon
...
Private Sub Form_Load()
    ...
    If bStat = True Then
        ...
        txtInput(0).Text = frmBook.lvList(0).SelectedItem.SubItems(1)
        txtInput(1).Text = GenerateID
    ...
    Else
        With frmBook
            txtInput(0).Text = frmBook.lvList(0).SelectedItem.SubItems(1)
            txtInput(1).Text = frmBook.lvList(1).SelectedItem.SubItems(1)
            txtInput(2).Text = frmBook.lvList(1).SelectedItem.SubItems(2)
            txtInput(3).Text = frmBook.lvList(1).SelectedItem.SubItems(5)
        End With
