...
Begin VB.Form frmSection 
   ...
   Begin VB.Frame fraList 
      ...
      Begin VB.Label Label2 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "You can view List of Sections"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
Private Sub btnAll_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_sections.sc_id, tbl_sections.sction, tbl_sections.dscrpt " & _
                    "FROM tbl_sections;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, Index, sSQL(Index), iStartPage(Index), iNoPage(Index), iIcon(Index), sColumns(Index), iLoop(Index), sColWidth(Index), sFields(Index), "No Current Section(s) Records.")
    End If
End Sub
...
Private Sub btnEdited_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_sections.sc_id, tbl_sections.sction, tbl_sections.dscrpt " & _
                    "From tbl_sections " & _
                    "WHERE (((tbl_sections.DateModified) between #" & Date & "# AND #" & Date & "#)) " & _
                    "GROUP BY tbl_sections.sc_id, tbl_sections.sction, tbl_sections.dscrpt;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, Index, sSQL(Index), iStartPage(Index), iNoPage(Index), iIcon(Index), sColumns(Index), iLoop(Index), sColWidth(Index), sFields(Index), "No Current Updated Section(s) Records.")
    End If
End Sub
...
Public Function PRINT_RECORD(Index As Integer)
    ...
    If Index = 0 Then
        ...
        Set dtrSection.DataSource = rsView1
        dtrSection.Show 1
    End If
End Function
...
Private Sub Form_Load()
    ...
    sSQL(0) = "SELECT tbl_sections.sc_id, tbl_sections.sction, tbl_sections.dscrpt " & _
            "FROM tbl_sections;"
    Lv_MainInfo
    lvList(0).Refresh
    frmMain.TabMainIni 2, "Sections", 8
    'ListOfDisabledBtn
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    frmMain.TabMainIni 1, "Sections", 8
End Sub
...
Private Sub lvList_DblClick(Index As Integer)
    If Index = 0 Then
        If iRec(Index) > 0 Then
            frmSection_AE.bStat = False
            Set frmSection_AE.fCur = Me
            frmSection_AE.Show 1
        End If
    End If
End Sub
...
Public Sub Lv_MainInfo()
    sColWidth(0) = "2000,3500,2500"
    sColumns(0) = "Section ID,Section Name,Description"
    iIcon(0) = 8
    iLoop(0) = CountSplitItem(sColumns(0), ",")
    sFields(0) = "sc_id,sction,dscrpt"
    sNoRec(0) = "No Current Sections Records."
    iStartPage(0) = 1
    iNoPage(0) = 75
...
Public Function btnNew_Load(Index As Integer)
     If Index = 0 Then
        sSQL(Index) = "SELECT tbl_sections.sc_id, tbl_sections.sction, tbl_sections.dscrpt " & _
                    "From tbl_sections " & _
                    "WHERE (((tbl_sections.DateAdded) between #" & Date & "# AND #" & Date & "#)) " & _
                    "GROUP BY tbl_sections.sc_id, tbl_sections.sction, tbl_sections.dscrpt;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, 0, sSQL(0), iStartPage(0), iNoPage(0), iIcon(0), sColumns(0), iLoop(0), sColWidth(0), sFields(0), "No Current New Sections Added Records.")
    End If
End Function
    ...
    If iRec(Index) > 0 Then
        If Index = 0 Then
            ...
            If MsgBox(sMsgDel, vbQuestion + vbYesNo, "Delete") = vbYes Then
                DELETE_DATA "tbl_sections", "sc_id", lvList(Index).SelectedItem.SubItems(1)
