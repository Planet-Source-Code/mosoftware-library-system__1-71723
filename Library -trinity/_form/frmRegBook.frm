...
Begin VB.Form frmRegBook 
   Caption         =   "Registered Book"
   ClientHeight    =   7815
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   9540
   ...
   Begin VB.Frame fraList 
      ...
      Begin VB.Label Label2 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "You can view All Registered Books and other Information."
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
Private Sub btnAll_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_reg_books.rb_id, tbl_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks " & _
            "FROM tbl_books INNER JOIN tbl_reg_books ON tbl_books.isbn = tbl_reg_books.isbn " & _
            "GROUP BY tbl_reg_books.rb_id, tbl_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, Index, sSQL(Index), iStartPage(Index), iNoPage(Index), iIcon(Index), sColumns(Index), iLoop(Index), sColWidth(Index), sFields(Index), "No Current Reg. Books Record.")
    End If
End Sub
...
Private Sub btnEdited_Click(Index As Integer)
    If Index = 0 Then
        sSQL(Index) = "SELECT tbl_reg_books.rb_id, tbl_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks " & _
                    "FROM tbl_books INNER JOIN tbl_reg_books ON tbl_books.isbn = tbl_reg_books.isbn " & _
                    "WHERE (((tbl_reg_books.DateModified) Between #" & Date & "# And #" & Date & "#)) " & _
                    "GROUP BY tbl_reg_books.rb_id, tbl_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks;"
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, Index, sSQL(Index), iStartPage(Index), iNoPage(Index), iIcon(Index), sColumns(Index), iLoop(Index), sColWidth(Index), sFields(Index), "No Current Updated Reg. Books Record.")
    End If
End Sub
...
Public Function PRINT_RECORD(Index As Integer)
    ...
    If Index = 0 Then
        ...
        Set dtrRegisterBooks.DataSource = rsView1
        dtrRegisterBooks.Show 1
    End If
End Function
...
Private Sub Form_Load()
    ...
    sSQL(0) = "SELECT tbl_reg_books.rb_id, tbl_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks " & _
            "FROM tbl_books INNER JOIN tbl_reg_books ON tbl_books.isbn = tbl_reg_books.isbn " & _
            "GROUP BY tbl_reg_books.rb_id, tbl_books.isbn, tbl_books.title, tbl_reg_books.barcode, tbl_reg_books.borrow, tbl_reg_books.pending, tbl_reg_books.remarks;"
    Lv_MainInfo
    lvList(0).Refresh
    frmMain.TabMainIni 2, "Reg. Books", 3
    'ListOfDisabledBtn
End Sub
...
Private Sub Form_Unload(Cancel As Integer)
    frmMain.TabMainIni 1, "Reg. Books", 3
End Sub
...
Private Sub lvList_DblClick(Index As Integer)
    If Index = 0 Then
        If iRec(Index) > 0 Then
            frmRegBook_AE.bStat = False
            Set frmRegBook_AE.fCur = Me
            frmRegBook_AE.Show 1
        End If
    End If
End Sub
...
Public Sub Lv_MainInfo()
    sColWidth(0) = "1700,1400,2500,1200,1000,1000,2500"
    sColumns(0) = "Registered ID,ISBN,Title,Barcode,Borrow,Pending,Remarks"
    iIcon(0) = 3
    iLoop(0) = CountSplitItem(sColumns(0), ",")
    sFields(0) = "rb_id,isbn,title,barcode,borrow,pending,remarks"
    sNoRec(0) = "No Current Reg. Books Record."
    iStartPage(0) = 1
    iNoPage(0) = 75
...
Public Function btnNew_Load(Index As Integer)
     If Index = 0 Then
        ...
        iStartPage(Index) = 1
        iRec(Index) = LvPageStat(Me, 0, sSQL(0), iStartPage(0), iNoPage(0), iIcon(0), sColumns(0), iLoop(0), sColWidth(0), sFields(0), "No Current New Reg. Books Added Record.")
    End If
End Function
    ...
    If iRec(Index) > 0 Then
        If Index = 0 Then
            ...
            If MsgBox(sMsgDel, vbQuestion + vbYesNo, "Delete") = vbYes Then
                DELETE_DATA "tbl_reg_books", "rb_id", lvList(Index).SelectedItem.SubItems(1)
